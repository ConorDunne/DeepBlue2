...
//import packages for scene color, text and stage
...
public class Spike {
  ...
  //initialise stack holding the counters
    Stack<Counter> stack = new Stack<>();

  //spike constructor that enters parameters into local variables
    public Spike(int number, int numberRev, int LM) {
        this.number = number;
        this.numberRev = numberRev;
        this.lm = LM;

      //adjusts positioning of the spikes
        if(number > 12) {
            yBase = 0.05;
            yPoint = 0.4;
        }
...
//  Initialize Special Positions
  ...
  //draws the spikes, sets color and text
  ...
  //adds counter to the spike stack
  ...
  //returns counter at top of spike
    public Counter topOfSpike(){
        return stack.peek();
    }
    
  //removes counter from spike and returns new number of counters on spike
    public Counter removeFromSpike() { return stack.pop(); }
    public int getSizeOfSpike(){
        return stack.size();
    }
    
  //accessor methods for all variables and check for empty spike
